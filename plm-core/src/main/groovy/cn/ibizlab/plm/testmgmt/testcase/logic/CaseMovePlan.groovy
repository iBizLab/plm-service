...
/**
 * 实体[TEST_CASE]处理逻辑[移入计划]运行时对象
 * 此代码用户功能扩展代码
 *
 * 将选中测试用例移入执行计划（在指定的执行用例内生成执行用例）
 */
class CaseMovePlan extends DELogicRuntime {

    private static final Log log = LogFactory.getLog(CaseMovePlan.class)
    ...
    /**
     * 执行逻辑节点[设置执行用例信息（附件置空）]，逻辑类型[PREPAREPARAM]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
