...
/**
 * 实体[TEST_CASE]处理逻辑[获取测试库成员]运行时对象
 * 此代码用户功能扩展代码
 *
 * 获取测试库成员信息，用于判断当前用户权限
 */
class GetLibraryMember extends DELogicRuntime {

    private static final Log log = LogFactory.getLog(GetLibraryMember.class)
    ...
    /**
     * 执行逻辑节点[获取测试库ID并设置过滤参数]，逻辑类型[PREPAREPARAM]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
    ...
    /**
     * 执行逻辑节点[查询当前用户是否为测试库成员]，逻辑类型[DEDATASET]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
