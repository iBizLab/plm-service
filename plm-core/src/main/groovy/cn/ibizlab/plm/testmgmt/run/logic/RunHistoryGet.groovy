...
/**
 * 实体[RUN]处理逻辑[执行结果获取]运行时对象
 * 此代码用户功能扩展代码
 *
 * 执行用例中，获取执行结果数据
 */
class RunHistoryGet extends DELogicRuntime {

    private static final Log log = LogFactory.getLog(RunHistoryGet.class)
    ...
    /**
     * 执行逻辑节点[获取执行用例关联的执行结果]，逻辑类型[RAWSQLCALL]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
    ...
    /**
     * 执行逻辑节点[填充执行结果列表]，逻辑类型[PREPAREPARAM]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
