...
/**
 * 实体[WORKLOAD]处理逻辑[团队维度工时日历查询]运行时对象
 * 此代码用户功能扩展代码
 *
 * 工时管理菜单中工时日历：团队维度查询数据使用
 */
class UserGroupDimension extends DELogicRuntime {

    private static final Log log = LogFactory.getLog(UserGroupDimension.class)
    ...
    /**
     * 执行逻辑节点[设置分页参数]，逻辑类型[PREPAREPARAM]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
    ...
    /**
     * 执行逻辑节点[查询团队工时记录]，逻辑类型[DEDATASET]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
