...
/**
 * 实体[WORKLOAD]处理逻辑[工作类别维度管理工时日历查询]运行时对象
 * 此代码用户功能扩展代码
 *
 * 工时管理菜单中工时日历：工作类型维度查询数据使用
 */
class TypeOfManagementDimension extends DELogicRuntime {

    private static final Log log = LogFactory.getLog(TypeOfManagementDimension.class)
    ...
    /**
     * 执行逻辑节点[重置分页参数]，逻辑类型[PREPAREPARAM]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
    ...
    /**
     * 执行逻辑节点[填充工作类别为无]，逻辑类型[PREPAREPARAM]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
    ...
    /**
     * 执行逻辑节点[查询工作类别工时]，逻辑类型[DEDATASET]
     * @param iDELogicSession
     * @param iPSDELogicNode
     * @throws Throwable
     */
